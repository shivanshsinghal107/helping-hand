...
def make_tables():
    db.execute('''CREATE TABLE IF NOT EXISTS gethelp (id INTEGER PRIMARY KEY AUTOINCREMENT, unique_id CHAR(8) NOT NULL,
    ...
    db.execute('''CREATE TABLE IF NOT EXISTS giveleads (id INTEGER PRIMARY KEY AUTOINCREMENT, unique_id CHAR(8) NOT NULL,
                  name VARCHAR(255) NOT NULL, state VARCHAR(64) NOT NULL, district VARCHAR(100) NOT NULL, date VARCHAR(30),
                  requirements VARCHAR(255), bgroup VARCHAR(5), phone VARCHAR(15), email VARCHAR(100), note text)''')
    db.commit()
    db.close()
