...
def make_tables():
    db.execute('''CREATE TABLE IF NOT EXISTS gethelp (id SERIAL PRIMARY KEY, unique_id CHAR(8) NOT NULL,
                  ...
                  bgroup VARCHAR(5), phone VARCHAR(15), email VARCHAR(100), note text, UNIQUE(unique_id))''')
    ...
    db.execute('''CREATE TABLE IF NOT EXISTS feedback (id SERIAL PRIMARY KEY, unique_id CHAR(8) NOT NULL,
                  note text NOT NULL, FOREIGN KEY(unique_id) REFERENCES gethelp(unique_id) ON DELETE CASCADE ON UPDATE CASCADE)''')
